...
if args.minsnvs > 0:
    ...
    if len(lowsnv_samples) > 0:
        ...
        lowsnv_fh.close()
        log.info(
            str(len(lowsnv_samples)) + " samples have fewer than " +
...
if (nsubtypes > samples.size and args.decomp == "pca"):
    ...
    if (args.groupvar):
        grouperr = " or use a grouping variable with >" + str(
            nsubtypes) + " levels"
    else:
        grouperr = ""
    log.error("You are trying to run a PCA with more dimensions (" +
              str(nsubtypes) + ") than samples (" + str(samples.size) +
              ")---unable to proceed with outlier detection!" +
...
if args.filtermode == "none":
    ...
    kd_lists = DetectOutliers(decomp_data.W, samples, args.filtermode,
                              threshold, projdir, args.seed)
...
if (args.report and args.matrixname == "subtype_count_matrix"):
    ...
    log.debug("Diagnostics config file written to: " + projdir +
              "/config.yaml")
...
if args.filterout:
    if (args.mode == "vcf" and not (args.input.lower().endswith(('.txt')))):
        log.debug("Filtering input VCF using sample file " +
                  paths['keep_path'])
        filterVCF(args.input, kd_lists.keep)

    elif args.mode == "txt":
        log.debug("Filtering input data using sample file " +
                  paths['keep_path'])
