...
plotmin <- floor(min(rsme_plot$RMSE)/0.005)*0.005
# ceiling(x/u)*u
...
ggplot()+
  ...
  geom_segment(data=rmse_plot,
    aes(x=ID, xend=ID, y=0, yend=RMSE, colour=signature))+
  geom_point(data=rmse_plot,
    aes(x=ID, y=RMSE, colour=signature), size = 4) +
  ...
  # scale_x_continuous(position = c("top", "bottom"))+
...
```{r err_profiles, echo=FALSE, fig.height = 16, fig.width = 10}
spectra_r <- spectra_rates %>%
  mutate_at(vars(-1), funs(normalize)) %>%
  gather(subtype, err, 2:ncol(spectra_rates)) %>%
  separate(subtype, c("category", "motif"), sep = "[.]")

outlier_r <- spectra_r %>%
  dplyr::filter(ID %in% rmse_plot$ID[1:10])
