...
packages <- c("dplyr","tidyr","ggplot2","yaml", "devtools", "knitr", "corrr", "viridis", "heatmaply")
...
# signature contributions across samples
sig_contribs_keep <- sig_contribs[complete.cases(sig_contribs),] %>%
  dplyr::filter(ID %in% keep_ids$V1) %>%
  mutate(sumVar = rowSums(.[2:ncol(sig_contribs)])) %>%
  mutate_at(vars(starts_with("S")), funs(./sumVar)) %>%
  mutate(maxgp = apply(.[,2:ncol(sig_contribs)], 1, function(x)
...
# signature contributions across samples
sig_contribs_drop <- sig_contribs[complete.cases(sig_contribs),] %>%
...
```{r rmse, echo=FALSE, fig.height = 12, fig.width = 10}
...
rmse_plot <- rmse_m %>%
  ...
  mutate(key="RMSE") %>%
  mutate(maxgp=factor(maxgp, levels=c("S1", "S2", "S3")))
  # top_n(contribution, 1)
  # # mutate(k2=-k2) %>%
...
ggplot()+
  geom_bar(data=contribution_plot,
    aes(x=ID, y=k2, fill=signature), stat="identity")+
  geom_text(data=contribution_plot,
    aes(x=ID, y=1, label=NMF_out), hjust=0)+
  # geom_text(data=d2a, aes(x=ID, y=1.1, label=ID)) +
  geom_segment(data=rmse_plot,
    aes(x=ID, xend=ID, y=0, yend=RMSE, colour=maxgp))+
  geom_point(data=rmse_plot,
    aes(x=ID, y=RMSE, colour=maxgp), size = 4) +
  geom_hline(yintercept=0, linetype="dashed")+
  scale_fill_viridis(discrete=TRUE, drop=FALSE) +
  scale_color_viridis(discrete=TRUE, drop=FALSE) +
  # scale_colour_manual(values=viridis_pal()(3)[c(2,3)])+
  # scale_colour_manual(values=viridis_pal()(3), drop=FALSE)+
...
```{r err_profiles, echo=FALSE, fig.height = 24, fig.width = 10}
...
ggplot(outlier_r20, aes(x=motif, y=err))+
  geom_bar(stat="identity")+
  geom_hline(yintercept=0, linetype="dashed")+
...
# wts <- rowMeans(outlier_r_m)
# hc <- hclust(dist(as.matrix(outlier_r_m)), method="ward.D2")
# row.order <- order.dendrogram(reorder(as.dendrogram(hc), wts))
#
# # row.order <- hclust(dist(as.matrix(outlier_r_m)), method="ward.D2")$order
#
# outlier_r2 <- outlier_r %>%
#   mutate(ID=factor(ID, levels=levels(ID)[row.order]))
#
# ggplot(outlier_r2, aes(x=motif, y=ID, fill=err))+
#   geom_tile()+
