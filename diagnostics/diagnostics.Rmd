...
ggplot(sig_contribs2,
    aes(x=ID, y=contribution,
      group=signature, colour=signature, fill=signature))+
  # geom_line()+
  geom_bar(stat="identity", position="stack")+
...
rmse <- rmse %>%
  mutate(ID=factor(ID, levels=ID[order(rmse)])) %>%
  arrange(desc(RMSE)) %>%
  head(50)
...
breaks <- c(round(min(rmse2$k2), 2), seq(0,1,0.25))

ggplot()+
  geom_bar(data=d2a[d2a$k1!="rmse",],
    aes(x=ID, y=k2, fill=key), stat="identity")+
  geom_text(data=d2a, aes(x=ID, y=0.1, label=ID)) +
  geom_bar(data=d2a[d2a$k1=="rmse",][1:3,],
    aes(x=ID, y=k2), stat="identity", width=0.05)+
  geom_point(data=d2a[d2a$k1=="rmse",][1:3,],
    aes(x=ID, y=k2), size = 6, colour="red") +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0,
    fill = "blue", alpha = .1, color = NA)+
  annotate("text", x = length(unique(d2a$ID))+0.8, y = min(d2a$k2)/2,
    label = "RMSE", size=10, vjust=1) +
  annotate("text", x = length(unique(d2a$ID))+0.8, y = 0.5,
    label = "Signature contribution", size=10, vjust=1) +
