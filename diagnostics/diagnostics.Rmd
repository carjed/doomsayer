...
packages <- c("dplyr","tidyr","ggplot2","yaml", "devtools", "knitr", "corrr")
...
if(nrow(spectra2)==12){
  match_b <- function(type){
    benchmark_props <- c(0.07298031, 0.27229427, 0.06980828,
                         0.10192701, 0.08812614, 0.39486398)
    names(benchmark_props) <- unique(spectra2$category)
    return(benchmark_props[type])
  }

  match_B <- Vectorize(match_b, vectorize.args = c('type'))

  benchmark <- spectra2 %>%
    dplyr::filter(group=="keep") %>%
    ungroup() %>%
    mutate(group="benchmark", count=0, prop=match_b(category))

  spectra2 <- rbind(spectra2, benchmark)
}
...
sig_contribs2 <- sig_contribs[complete.cases(sig_contribs),] %>%
  mutate(V1 = ID, study=substr(V1, 1, 4)) %>%  #
  mutate(ID=gsub("[[:punct:]]", "", V1)) %>%
  mutate(ID=gsub("[[:alpha:]]", "", ID)) %>%
  mutate(ID=as.numeric(ID)) %>%
  mutate(sumVar = rowSums(.[2:ncol(sig_contribs)])) %>%
  rowwise() %>%
  mutate_each(funs(./sumVar), -c(ID, V1, study)) %>%
  dplyr::select(-sumVar) %>%
  gather(signature, contribution, -c(ID, V1, study)) %>%
  group_by(study) %>%
  arrange(study, ID)

ggplot(sig_contribs2,
  aes(x=ID, y=contribution, group=signature, colour=signature, fill=signature))+
  # geom_line()+
...
sig_contribs2 <- sig_contribs[complete.cases(sig_contribs),] %>%
  ...
  rowwise() %>%
  mutate_each(funs(./sumVar), -c(ID,maxgp,V1,study)) %>%
  dplyr::select(-sumVar) %>%
  gather(signature, contribution, -c(ID,maxgp,V1,study)) %>%
  mutate(contribution=ifelse((ID %in% drop_ids$V1), contribution, 0)) %>%
  group_by(study) %>%
  arrange(study, ID)
  #%>%
  # mutate(ID=factor(ID, levels=ID[order(contribution)]))
