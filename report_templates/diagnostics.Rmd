...
If any component shows clear evidence of multimodality, consider changing the default outlier detection from `--f ee` (elliptic envelope) to either `--f lof` (local outlier factor) or `--f if` (isolation forest)
...
Fold-difference in contribution per subtype for each outlier. Fold-differences are calculated relative to the mean contribution of that subtype across all samples. Scale is truncated to +/-3-fold difference
